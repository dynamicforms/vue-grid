<template>
  <div class="df-grid-sorting-indicator-wrapper">
    ...
    <svg v-if="sortable" :viewBox="viewBox" stroke="currentColor" fill="currentColor">
      ...
      <path v-if="direction !== 'asc'" data-sort="desc" d="M16,68l-16 -20 16 6 16 -6Z" />
    </svg>
  </div>
</template>
...
const props = withDefaults(defineProps<ColumnSortState>(), { direction: undefined, index: undefined, sortable: true });

const viewBox = computed(
  () => `0 ${props.direction === 'desc' ? '16' : '0'} 32 ${props.direction === 'asc' ? '52' : '68'}`,
);
</script>
